...
@router.post(
    "/{submission_id}/fact-check",
    status_code=status.HTTP_201_CREATED,
    summary="Create fact-check from submission",
    description="Create a fact-check from a submission's first claim and optionally assign a rating",
)
async def create_fact_check_from_submission(
    submission_id: UUID,
    ...
    """
    Create a fact-check from submission's first claim.

    This is a convenience endpoint to streamline the workflow.
    """
    from app.models.fact_check import FactCheck
    from app.models.claim import Claim

    # Check permission: only reviewers, admins, and super admins
    if current_user.role not in [UserRole.REVIEWER, UserRole.ADMIN, UserRole.SUPER_ADMIN]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only reviewers and admins can create fact-checks",
        )
    ...
    # Get first claim from submission
    result = await db.execute(
        select(Claim)
        .join(submission_claims)
        .where(submission_claims.c.submission_id == submission_id)
        .limit(1)
    )
    ...
    if not claim:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Submission has no claims to create fact-check from",
        )

    # Create fact-check
    fact_check = FactCheck(
        claim_id=claim.id,
        verdict="unverified",  # Default, will be updated when rating is assigned
        confidence=0.0,
        reasoning="Fact-check in progress",
        sources=[],
        sources_count=0,
    )
    db.add(fact_check)
...
@router.post(
    ...
    status_code=status.HTTP_201_CREATED,
    summary="Create fact-check and assign rating to submission",
    description="Convenience endpoint: creates fact-check from submission's first claim and assigns rating",
)
async def create_fact_check_and_rating(
    submission_id: UUID,
    rating_data: dict,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
    ...
    """
    Create a fact-check and assign rating in one step.

    Expects: {"rating": "true/false/...", "justification": "..."}
    """
    from app.models.fact_check import FactCheck
    from app.models.claim import Claim
    ...
    if current_user.role not in [UserRole.REVIEWER, UserRole.ADMIN, UserRole.SUPER_ADMIN]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only reviewers and admins can assign ratings",
        )

    # Get submission and first claim
    submission = await submission_service.get_submission(db, submission_id)
    if not submission:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if not claim:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Submission has no claims",
        )

    # Create fact-check if it doesn't exist
    ...
    if not fact_check:
        fact_check = FactCheck(
            claim_id=claim.id,
            verdict=rating_data.get("rating", "unverified"),
            confidence=0.8,
            reasoning=rating_data.get("justification", ""),
            sources=[],
            sources_count=0,
        )
        db.add(fact_check)
